 ...
 */
export function sortQuestionByActive(questions, answers) {
    questions = [...questions];
    questions.sort((a, b) => {
        if (b.ansIds.length === 0) return 1;
        if (a.ansIds.length === 0) return -1;
        return answers.filter(answer => answer.aid === b.ansIds[b.ansIds.length - 1])[0].ansDate - answers.filter(answer => answer.aid === a.ansIds[a.ansIds.length - 1])[0].ansDate;
    });
    return questions;
}
...
export function sortQuestionByUnanswered(questions) {
    questions = [...questions];
    return questions.filter(question => question.ansIds.length === 0);
}
