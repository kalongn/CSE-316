...
function EditQuestions() {
    ...
    useEffect(() => {
        ...
        Axios.patch(`${expressServer}/question`, { id: qid })
            .then(res => {
    ...
    function getDifferenceFromList1(list1, list2) {
        return list1.filter(item => !list2.includes(item));
    }

    function getDifferenceFromList2(list1, list2) {
    ...
    async function submit() {
        ...
        for (let tag of existTags) {
            if (tag.used_by.includes(uid)) {
                continue;
            }
            tag.used_by.push(uid);
        }

        const existTagsID = existTags.map(tag => tag._id);
        const existInNewTagsID = getDifferenceFromList1(existTagsID, question.tags.map(tag => tag._id)); // new exist tags
        const notExistInNewTagsID = getDifferenceFromList2(existTagsID, question.tags.map(tag => tag._id)); // removed exist tags

        const needUpdateexistTag = existTags.filter(tag => existInNewTagsID.includes(tag._id));
        ...
        await Axios.patch(`${expressServer}/tags`, { tags: needUpdateexistTag });
        await Axios.patch(`${expressServer}/profile/question/existTags/${qid}/${uid}`, { tags: notExistInNewTagsID });
        console.log(newTags);
        const { newTagID } = (await Axios.post(`${expressServer}/newtags`, { tags: newTags, user: currentUser._id })).data;
        console.log(newTagID);
        const allTagsID = [...existTagsID, ...newTagID];

        await Axios.patch(`${expressServer}/profile/question/${question._id}`, { title, summary, text, tags: allTagsID });
        navigate(`/profile/${uid}`);
    }

    function deleteQuestion() {
        Axios.delete(`${expressServer}/profile/question/${qid}/${uid}`)
            .then(res => {
                navigate(`/profile/${uid}`);
            })
            .catch(err => {
    ...
    return (
        <div className='page-wrapper new-question-page-wrapper'>
            <div className='question-section-wrapper'>
                ...
                <input className='question-section-input' type="text" defaultValue={title} placeholder='e.g. How to Pass CSE 316 Fundamental Of Software Development' onChange={(e) => { setTitle(e.target.value.trim()) }} />
            ...
            <div className='question-section-wrapper'>
                ...
                <textarea className='question-section-input' type="text" defaultValue={summary} rows={3} onChange={(e) => { setSummary(e.target.value.trim()) }} />
            ...
            <div className='question-section-wrapper'>
                ...
                <textarea className='question-section-input' type="text" defaultValue={text} rows={6} onChange={(e) => { setText(e.target.value.trim()) }} />
            ...
            <div className='question-section-wrapper'>
                ...
                <input className='question-section-input' type="text" defaultValue={tags} placeholder='e.g. react javascript stonybrook' onChange={(e) => { setTags(e.target.value.trim()) }} />
                <div className='question-section-error'>{tagsCorrect ? "" : tagsErrorMessage}</div>
            </div>
            <div className='question-button-wrapper'>
                <button className='submit-button bold delete-bttn' onClick={deleteQuestion}>Delete</button>
